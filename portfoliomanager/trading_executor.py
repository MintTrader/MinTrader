...
class TradingExecutor:
    ...
    def _build_trading_prompt(
        ...
        prompt_parts.append("")
        prompt_parts.append("🎯 YOUR PRIMARY OBJECTIVE:")
        prompt_parts.append("Manage a DIVERSE PORTFOLIO that MAXIMIZES PROFITS while MINIMIZING RISKS through:")
        prompt_parts.append("  • 📊 SECTOR DIVERSIFICATION - Hold positions across MULTIPLE sectors (tech, healthcare, finance, energy, etc.)")
        prompt_parts.append("  • 🥚 CONCENTRATION MANAGEMENT - NEVER put all eggs in one basket! Spread capital across multiple stocks")
        prompt_parts.append("  • ⚖️  POSITION SIZING - No single position should dominate the portfolio (aim for balanced allocation)")
        prompt_parts.append("  • 🛡️  RISK MITIGATION - Diversification protects against individual stock failures")
        prompt_parts.append("")
        prompt_parts.append("You manage this entire portfolio and decide how many positions to hold and which stocks to trade.")
        prompt_parts.append("The analyzed stocks are RESEARCH to help you, but YOU make ALL trading decisions based on:")
        prompt_parts.append("  • Your analysis research (if available)")
        prompt_parts.append("  • Current portfolio state (positions, P&L, cash)")
        prompt_parts.append("  • Portfolio diversification and sector balance")
        ...
        prompt_parts.append("  ✅ Make decisions on stocks NOT analyzed")
